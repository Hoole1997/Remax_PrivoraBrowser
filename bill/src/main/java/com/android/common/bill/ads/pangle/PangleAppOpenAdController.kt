...
class PangleAppOpenAdController private constructor() {
    ...
    fun getCachedAdPeek(adUnitId: String? = null): PAGAppOpenAd? {
    ...
    fun getCurrentAd(adUnitId: String? = null): PAGAppOpenAd? = getCachedAdPeek(adUnitId)
        ...
        return suspendCancellableCoroutine { continuation ->
            ...
            appOpenAd.setAdInteractionListener(object : PAGAppOpenAdInteractionCallback() {
                ...
                override fun onAdClicked() {
                    ...
                    AdEventReporter.reportClick(AdType.APP_OPEN, AdPlatform.PANGLE, adUnitId, currentAdUniqueId, totalClickCount, currentAdSource ?: AdPlatform.PANGLE.key, appOpenAd.pagRevenueInfo?.showEcpm?.revenue?.toDoubleOrNull() ?: 0.0, currentCurrency ?: "USD", sessionId = sessionId)
                }
 
                override fun onAdDismissed() {
                    totalCloseCount++
                    AdLogger.d("Pangle开屏广告关闭")
                    AdEventReporter.reportClose(AdType.APP_OPEN, AdPlatform.PANGLE, adUnitId, totalCloseCount, currentAdSource ?: AdPlatform.PANGLE.key, appOpenAd.pagRevenueInfo?.showEcpm?.revenue?.toDoubleOrNull() ?: 0.0, currentCurrency ?: "USD", sessionId = sessionId)
