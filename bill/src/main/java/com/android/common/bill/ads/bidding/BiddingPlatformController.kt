...
object BiddingPlatformController {
    ...
    fun isPlatformEnabled(adType: AdType, platform: AdPlatform): Boolean {
        ...
        // 2. 检查宿主是否注入了平台初始化所需的关键配置
        val missingFields = PlatformRuntimeConfigChecker.getMissingRequiredFields(platform)
        if (missingFields.isNotEmpty()) {
            AdLogger.d("${platform.key} 配置已启用但缺少运行时配置，跳过参与竞价: ${missingFields.joinToString()}")
            return false
        }
    ...
    fun isGamEnabled(adType: AdType): Boolean = isPlatformEnabled(adType, AdPlatform.GAM)
