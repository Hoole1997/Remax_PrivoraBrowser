        ...
        return suspendCancellableCoroutine { continuation ->
            ...
            InterstitialAd.load(adRequest, object : AdLoadCallback<InterstitialAd> {
                override fun onAdLoaded(ad: InterstitialAd) {
                    ...
                    totalLoadSucCount++
                    AdEventReporter.reportLoaded(AdType.INTERSTITIAL, AdPlatform.ADMOB, adUnitId, totalLoadSucCount, ad.getResponseInfo().loadedAdSourceResponseInfo?.name.orEmpty(), ceil(loadTime / 1000.0).toInt(), requestId)
                ...
                override fun onAdFailedToLoad(adError: LoadAdError) {
                    ...
                    totalLoadFailCount++
                    AdEventReporter.reportLoadFail(AdType.INTERSTITIAL, AdPlatform.ADMOB, adUnitId, totalLoadFailCount, adError.responseInfo?.loadedAdSourceResponseInfo?.name.orEmpty(), ceil(loadTime / 1000.0).toInt(), adError.message, requestId)
        ...
        return suspendCancellableCoroutine { continuation ->
            ...
            interstitialAd.adEventCallback = object : InterstitialAdEventCallback {
                ...
                override fun onAdPaid(value: AdValue) {
                    ...
                    AdEventReporter.reportImpression(AdType.INTERSTITIAL, AdPlatform.ADMOB, adUnitId, currentAdUniqueId, totalShowCount, interstitialAd.getResponseInfo().loadedAdSourceResponseInfo?.name.orEmpty(), currentAdValue?.let { it.valueMicros / 1_000_000.0 } ?: 0.0, currentAdValue?.currencyCode ?: "", sessionId = sessionId, isPreload = isPreload)
                    
                    // 上报真实的广告收益数据
                    AdRevenueReporter.reportRevenue(AdType.INTERSTITIAL, AdPlatform.ADMOB, adUnitId, value.valueMicros / 1_000_000.0, value.currencyCode, interstitialAd.getResponseInfo().loadedAdSourceResponseInfo?.name ?: "Admob", interstitialAd.getResponseInfo().loadedAdSourceResponseInfo?.instanceName.orEmpty())
                }
                ...
                override fun onAdDismissedFullScreenContent() {
                    ...
                    AdEventReporter.reportClose(AdType.INTERSTITIAL, AdPlatform.ADMOB, adUnitId, totalCloseCount, interstitialAd.getResponseInfo().loadedAdSourceResponseInfo?.name.orEmpty(), currentAdValue?.let { it.valueMicros / 1_000_000.0 } ?: 0.0, currentAdValue?.currencyCode ?: "", sessionId = sessionId)
                ...
                override fun onAdClicked() {
                    ...
                    AdEventReporter.reportClick(AdType.INTERSTITIAL, AdPlatform.ADMOB, adUnitId, currentAdUniqueId, totalClickCount, interstitialAd.getResponseInfo().loadedAdSourceResponseInfo?.name.orEmpty(), currentAdValue?.let { it.valueMicros / 1_000_000.0 } ?: 0.0, currentAdValue?.currencyCode ?: "", sessionId = sessionId)
                }
