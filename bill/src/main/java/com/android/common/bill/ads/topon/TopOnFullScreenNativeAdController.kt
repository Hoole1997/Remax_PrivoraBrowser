...
// 注册 Activity 销毁时的清理回调
                ...
                is AdResult.Success -> {
                    ...
                    nativeAd.setDislikeCallbackListener(object : TUNativeDislikeListener() {
                        override fun onAdCloseButtonClick(
                            ...
                            AdEventReporter.reportClose(AdType.FULL_SCREEN_NATIVE, AdPlatform.TOPON, finalPlacementId, totalCloseCount, adInfo.networkName ?: "", revenueValue, revenueCurrency, sessionId = currentSessionId)
                        }
                    })
        ...
        return object : TUNativeEventListener {
            ...
            override fun onAdImpressed(view: TUNativeAdView, adInfo: TUAdInfo) {
                ...
                if (!isCacheFull(placementId)) {
                    val reloadSessionId = sessionId
                    CoroutineScope(SupervisorJob() + Dispatchers.IO).launch {
                        try {
                            AdEventReporter.withLoadPosition(AdEventReporter.findTrackedPositionBySessionId(reloadSessionId)
                            ) {
            ...
            override fun onAdClicked(view: TUNativeAdView, adInfo: TUAdInfo) {
                ...
                AdEventReporter.reportClick(AdType.FULL_SCREEN_NATIVE, AdPlatform.TOPON, placementId, currentAdUniqueId, totalClickCount, adInfo.networkName ?: "", revenueValue, revenueCurrency, sessionId = sessionId)
            }
    ...
    fun closeEvent(placementId: String = "") {
        ...
        AdEventReporter.reportClose(AdType.FULL_SCREEN_NATIVE, AdPlatform.TOPON, placementId, totalCloseCount, currentAdInfo?.networkName ?: "", currentAdInfo?.publisherRevenue ?: 0.0, currentAdInfo?.currency ?: "USD", sessionId = currentSessionId)
    }
    ...
    fun getCachedAdPeek(placementId: String? = null): TUNative? {
        val finalPlacementId = placementId ?: BillConfig.topon.fullNativeId
