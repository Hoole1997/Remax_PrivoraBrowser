...
object PreloadController {
    ...
    fun preloadAll(context: Context) {
        ...
        scope.launch { preloadGamInternal(context) }
    ...
    fun preloadAllFullScreenNative(context: Context) {
        ...
        scope.launch { preloadGamFullScreenNativeInternal(context) }
    ...
    private suspend fun preloadAdmobInterstitialInternal(context: Context) {
        if (!BiddingPlatformController.isPlatformEnabled(AdType.INTERSTITIAL, AdPlatform.ADMOB)) return
        try {
    ...
    private suspend fun preloadAdmobNativeInternal(context: Context) {
        if (!BiddingPlatformController.isPlatformEnabled(AdType.NATIVE, AdPlatform.ADMOB)) return
        try {
    ...
    private suspend fun preloadAdmobFullScreenNativeInternal(context: Context) {
        if (!BiddingPlatformController.isPlatformEnabled(AdType.FULL_SCREEN_NATIVE, AdPlatform.ADMOB)) return
        try {
    ...
    fun preloadGam(context: Context) {
        scope.launch { preloadGamInternal(context) }
    }

    fun preloadGamInterstitial(context: Context) {
        scope.launch { preloadGamInterstitialInternal(context) }
    }

    fun preloadGamNative(context: Context) {
        scope.launch { preloadGamNativeInternal(context) }
    }

    fun preloadGamFullScreenNative(context: Context) {
        scope.launch { preloadGamFullScreenNativeInternal(context) }
    }

    private suspend fun preloadGamInternal(context: Context) {
        preloadGamNativeInternal(context)
        preloadGamInterstitialInternal(context)
    }

    private suspend fun preloadGamInterstitialInternal(context: Context) {
        if (!BiddingPlatformController.isPlatformEnabled(AdType.INTERSTITIAL, AdPlatform.GAM)) return
        try {
            AdLogger.d("GAM插页开始异步预加载，广告位ID: %s", BillConfig.gam.interstitialId)
            GamInterstitialAdController.getInstance().preloadAd(context, BillConfig.gam.interstitialId)
        } catch (e: Exception) {
            AdLogger.e("GAM插页异步预加载广告失败", e)
        }
    }

    private suspend fun preloadGamNativeInternal(context: Context) {
        if (!BiddingPlatformController.isPlatformEnabled(AdType.NATIVE, AdPlatform.GAM)) return
        try {
            AdLogger.d("GAM原生开始异步预加载，广告位ID: %s", BillConfig.gam.nativeId)
            GamNativeAdController.getInstance().preloadAd(context, BillConfig.gam.nativeId)
        } catch (e: Exception) {
            AdLogger.e("GAM原生异步预加载广告失败", e)
        }
    }

    private suspend fun preloadGamFullScreenNativeInternal(context: Context) {
        if (!BiddingPlatformController.isPlatformEnabled(AdType.FULL_SCREEN_NATIVE, AdPlatform.GAM)) return
        try {
            AdLogger.d("GAM全屏原生开始异步预加载，广告位ID: %s", BillConfig.gam.fullNativeId)
            GamFullScreenNativeAdController.getInstance().preloadAd(context, BillConfig.gam.fullNativeId)
        } catch (e: Exception) {
            AdLogger.e("GAM全屏原生异步预加载广告失败", e)
        }
    }
