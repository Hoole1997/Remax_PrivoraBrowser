...
// 注册 Activity 销毁时的清理回调
            ...
            try {
                ...
                bannerView.setBannerAdListener(object : TUBannerListener {
                    ...
                    override fun onBannerClicked(adInfo: TUAdInfo) {
                        ...
                        AdEventReporter.reportClick(AdType.BANNER, AdPlatform.TOPON, placementId, currentAdUniqueId, totalClickCount, adInfo.networkName ?: "", revenueValue, revenueCurrency, sessionId = currentSessionId)
                    }
                    ...
                    override fun onBannerClose(adInfo: TUAdInfo) {
                        ...
                        AdEventReporter.reportClose(AdType.BANNER, AdPlatform.TOPON, placementId, totalCloseCount, adInfo.networkName ?: "", revenueValue, revenueCurrency, sessionId = currentSessionId)
                    }
    ...
    fun getCachedAdPeek(placementId: String? = null): TUBannerView? {
        val finalPlacementId = placementId ?: BillConfig.topon.bannerId
        return synchronized(adCachePool) {
            adCachePool.firstOrNull { it.placementId == finalPlacementId && !it.isExpired() }?.bannerView
        }
    }

    fun peekCachedAd(placementId: String = BillConfig.topon.bannerId): TUBannerView? = getCachedAdPeek(placementId)

    fun getCurrentAd(placementId: String? = null): TUBannerView? {
        return getCachedAdPeek(placementId)
    }

    fun hasCachedAd(placementId: String? = null): Boolean {
        return if (placementId != null) {
            getCachedAdPeek(placementId) != null
        } else {
            synchronized(adCachePool) { adCachePool.any { !it.isExpired() } }
        }
    }
