...
// 注册 Activity 销毁时的清理回调
                ...
                if (bannerView != null) {
                    ...
                    bannerAd.setAdInteractionListener(object : PAGBannerAdInteractionCallback() {
                        ...
                        override fun onAdClicked() {
                            ...
                            AdEventReporter.reportClick(AdType.BANNER, AdPlatform.PANGLE, finalAdUnitId, currentAdUniqueId, totalClickCount, currentAdSource ?: AdPlatform.PANGLE.key, bannerAd.pagRevenueInfo?.showEcpm?.revenue?.toDoubleOrNull() ?: 0.0, currentCurrency ?: "USD", sessionId = currentSessionId)
                        }
                        ...
                        override fun onAdDismissed() {
                            ...
                            AdEventReporter.reportClose(AdType.BANNER, AdPlatform.PANGLE, finalAdUnitId, totalCloseCount, currentAdSource ?: AdPlatform.PANGLE.key, bannerAd.pagRevenueInfo?.showEcpm?.revenue?.toDoubleOrNull() ?: 0.0, currentCurrency ?: "USD", sessionId = currentSessionId)
                        }
    ...
    private fun peekCachedAd(adUnitId: String): CachedBannerAd? {
        synchronized(adCachePool) {
            val index = adCachePool.indexOfFirst { it.adUnitId == adUnitId && !it.isExpired() && it.ad.isReady }
    ...
    fun getCachedAdPeek(adUnitId: String? = null): PAGBannerAd? {
        val finalAdUnitId = adUnitId ?: BillConfig.pangle.bannerId
        return peekCachedAd(finalAdUnitId)?.ad
    }

    fun getCurrentAd(adUnitId: String? = null): PAGBannerAd? = getCachedAdPeek(adUnitId)
