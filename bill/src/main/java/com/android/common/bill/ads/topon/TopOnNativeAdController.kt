...
// 注册 Activity 销毁时的清理回调
                ...
                is AdResult.Success -> {
                    ...
                    nativeAd.setDislikeCallbackListener(object :TUNativeDislikeListener(){
                        override fun onAdCloseButtonClick(
                            ...
                            AdEventReporter.reportClose(AdType.NATIVE, AdPlatform.TOPON, finalPlacementId, totalCloseCount, adInfo.networkName ?: "", revenueValue, revenueCurrency, sessionId = showSessionId)
                        }
                    })
                    // 设置容器高度（如果style指定了高度）
                    if (style.heightDp > 0) {
    ...
    private fun createNativeEventListener(
        ...
        return object : TUNativeEventListener {
            ...
            override fun onAdClicked(view: com.thinkup.nativead.api.TUNativeAdView, adInfo: TUAdInfo) {
                ...
                AdEventReporter.reportClick(AdType.NATIVE, AdPlatform.TOPON, placementId, currentAdUniqueId, totalClickCount, adInfo.networkName ?: "", revenueValue, revenueCurrency, sessionId = sessionId)
            }
     ...
     */
    @Deprecated(
        message = "当前库内未使用该接口，优先使用 peekCachedAd(BillConfig.topon.nativeId) 读取缓存，避免误用为统一竞价入口",
        replaceWith = ReplaceWith("peekCachedAd(BillConfig.topon.nativeId)"),
        level = DeprecationLevel.WARNING
    )
    fun getCurrentAd(): TUNative? {
        return peekCachedAd(BillConfig.topon.nativeId)
    }
