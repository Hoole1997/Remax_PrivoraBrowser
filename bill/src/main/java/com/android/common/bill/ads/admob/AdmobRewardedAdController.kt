        ...
        return suspendCancellableCoroutine { continuation ->
            ...
            RewardedAd.load(adRequest, object : AdLoadCallback<RewardedAd> {
                override fun onAdLoaded(rewardedAd: RewardedAd) {
                    ...
                    totalLoadSucCount++
                    AdEventReporter.reportLoaded(AdType.REWARDED, AdPlatform.ADMOB, adUnitId, totalLoadSucCount, rewardedAd.getResponseInfo().loadedAdSourceResponseInfo?.name.orEmpty(), ceil(loadTime / 1000.0).toInt(), requestId)
                ...
                override fun onAdFailedToLoad(loadAdError: LoadAdError) {
                    ...
                    totalLoadFailCount++
                    AdEventReporter.reportLoadFail(AdType.REWARDED, AdPlatform.ADMOB, adUnitId, totalLoadFailCount, loadAdError.responseInfo?.loadedAdSourceResponseInfo?.name.orEmpty(), ceil(loadTime / 1000.0).toInt(), loadAdError.message, requestId)
        ...
        return suspendCancellableCoroutine { continuation ->
            ...
            rewardedAd.adEventCallback = object : RewardedAdEventCallback {
                ...
                override fun onAdPaid(adValue: AdValue) {
                    ...
                    AdEventReporter.reportImpression(AdType.REWARDED, AdPlatform.ADMOB, adUnitId, currentAdUniqueId, totalShowCount, rewardedAd.getResponseInfo().loadedAdSourceResponseInfo?.name.orEmpty(), currentAdValue?.let { it.valueMicros / 1_000_000.0 } ?: 0.0, currentAdValue?.currencyCode ?: "", sessionId = sessionId, isPreload = isPreload)

                    // 上报真实的广告收益数据
                    AdRevenueReporter.reportRevenue(AdType.REWARDED, AdPlatform.ADMOB, adUnitId, adValue.valueMicros / 1_000_000.0, adValue.currencyCode, rewardedAd.getResponseInfo().loadedAdSourceResponseInfo?.name ?: "Admob", rewardedAd.getResponseInfo().loadedAdSourceResponseInfo?.instanceName.orEmpty())
                }
                ...
                override fun onAdClicked() {
                    ...
                    AdEventReporter.reportClick(AdType.REWARDED, AdPlatform.ADMOB, adUnitId, currentAdUniqueId, totalClickCount, rewardedAd.getResponseInfo().loadedAdSourceResponseInfo?.name.orEmpty(), currentAdValue?.let { it.valueMicros / 1_000_000.0 } ?: 0.0, currentAdValue?.currencyCode ?: "", sessionId = sessionId)
                }
            ...
            rewardedAd.show(activity) { rewardItem ->
                ...
                AdEventReporter.reportRewardEarned(AdType.REWARDED, AdPlatform.ADMOB, adUnitId, totalRewardEarnedCount, rewardedAd.getResponseInfo().loadedAdSourceResponseInfo?.name.orEmpty(), rewardItem.type, rewardItem.amount, sessionId = sessionId)
