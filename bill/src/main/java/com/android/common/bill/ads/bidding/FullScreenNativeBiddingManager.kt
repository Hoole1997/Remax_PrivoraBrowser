...
object FullScreenNativeBiddingManager {
    ...
    suspend fun bidding(
        ...
        gamAdUnitId: String = BillConfig.gam.fullNativeId,
    ...
    private suspend fun performBidding(
        ...
        val gamController = GamFullScreenNativeAdController.getInstance()
        ...
        val gamConfigEnabled = BiddingPlatformController.isGamEnabled(AdType.FULL_SCREEN_NATIVE)
        ...
        val gamEnabled = gamConfigEnabled && BiddingExclusionController.canPlatformBid(AdType.FULL_SCREEN_NATIVE, AdPlatform.GAM)
        ...
        AdLogger.d("全屏原生缓存状态 -> AdMob:$admobHasCache, GAM:$gamHasCache, Pangle:$pangleHasCache, TopOn:$toponHasCache, anyCached:$anyCached, 需加载:${needLoadPlatforms.map { it.key }}")

        // 上报无缓存平台
        if (admobEnabled && !admobHasCache) AdEventReporter.reportNoCache(AdType.FULL_SCREEN_NATIVE, AdPlatform.ADMOB, admobAdUnitId, position = position)
        if (gamEnabled && !gamHasCache) AdEventReporter.reportNoCache(AdType.FULL_SCREEN_NATIVE, AdPlatform.GAM, gamAdUnitId, position = position)
        if (pangleEnabled && !pangleHasCache) AdEventReporter.reportNoCache(AdType.FULL_SCREEN_NATIVE, AdPlatform.PANGLE, pangleAdUnitId, position = position)
        if (toponEnabled && !toponHasCache) AdEventReporter.reportNoCache(AdType.FULL_SCREEN_NATIVE, AdPlatform.TOPON, toponPlacementId, position = position)
        ...
        val biddingLog = String.format(
            Locale.US,
            "全屏原生竞价结果 -> AdMob: %.8f 美元%s, GAM: %.8f 美元%s, Pangle: %.8f 美元%s, TopOn: %.8f 美元%s",
