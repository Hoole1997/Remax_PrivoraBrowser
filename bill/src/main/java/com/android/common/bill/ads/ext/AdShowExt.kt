...
object AdShowExt {
    ...
    suspend fun showAppOpenAd(
        ...
        val sessionId = AdPositionTracker.trackSplashAdPosition(position)
        ...
        val (appOpenResult, interstitialResult) = coroutineScope {
            val appOpenDeferred = async { AppOpenBiddingManager.bidding(activity, position = position) }
            val interstitialDeferred = if (competeWithInterstitial) {
                async { InterstitialBiddingManager.bidding(activity, position = position) }
            } else null
        ...
        if (competeWithInterstitial && interstitialResult != null) {
            ...
            if (interstitialResult.eCpm > appOpenResult.eCpm) {
                ...
                return showInterstitialAd(activity, biddingResult = interstitialResult, ignoreFullNative = true, position = position)
            }
        }
        ...
        return when (appOpenResult.winner) {
            ...
            BiddingWinner.GAM -> {
                AdLogger.d("使用 GAM 展示开屏广告")
                AdEventReporter.withLoadPosition(position) {
                    GamAppOpenAdController.getInstance().showAd(
                        activity,
                        BillConfig.gam.splashId,
                        sessionId = sessionId
                    )
                }
            }
    ...
    suspend fun showInterstitialAd(
        ...
        if (shouldRedirectInterstitialToFullNative(ignoreFullNative, activity)) {
            return showFullScreenNativeAdInContainer(activity, true, position)
        }

        // 记录广告位置，生成 session_id
        val sessionId = AdPositionTracker.trackInterstitialAdPosition(position)
        ...
        val winner = if (biddingResult != null) {
            ...
            AdLogger.d("插页广告竞价开始")
            val result = InterstitialBiddingManager.bidding(activity, position = position)
        ...
        val result = when (winner) {
            ...
            BiddingWinner.GAM -> {
                AdLogger.d("使用 GAM 展示插页广告")
                AdEventReporter.withLoadPosition(position) {
                    GamInterstitialAdController.getInstance().showAd(
                        activity,
                        BillConfig.gam.interstitialId,
                        ignoreFullNative = ignoreFullNative,
                        sessionId = sessionId
                    )
                }
    ...
    suspend fun showRewardedAd(
        ...
        AdLogger.d("激励广告竞价开始, competeWithInterstitial=$competeWithInterstitial")

        // 记录广告位置，生成 session_id
        val sessionId = AdPositionTracker.trackRewardedAdPosition(position)
        ...
        // 并行获取激励和插页的竞价结果
        val (rewardedResult, interstitialResult) = coroutineScope {
            val rewardedDeferred = async { RewardedBiddingManager.biddingResult(activity, position = position) }
            val interstitialDeferred = if (competeWithInterstitial) {
                async { InterstitialBiddingManager.bidding(activity, position = position) }
            } else null

            Pair(rewardedDeferred.await(), interstitialDeferred?.await())
        }
        val winner = rewardedResult.winner
        AdLogger.d("激励广告竞价结果: winner=${rewardedResult.winner}, eCpm=${rewardedResult.eCpm}")

        if (competeWithInterstitial && interstitialResult != null) {
            AdLogger.d("插页广告竞价结果: winner=${interstitialResult.winner}, eCpm=${interstitialResult.eCpm}")

            // 如果插页 eCPM 更高，展示插页广告
            if (interstitialResult.eCpm > rewardedResult.eCpm) {
                AdLogger.d("插页广告 eCPM (${interstitialResult.eCpm}) > 激励广告 eCPM (${rewardedResult.eCpm}), 展示插页广告")
        ...
        return when (winner) {
            ...
            BiddingWinner.GAM -> {
                AdLogger.d("使用 GAM 展示激励广告")
                AdEventReporter.withLoadPosition(position) {
                    GamRewardedAdController.getInstance().showAd(
                        activity,
                        BillConfig.gam.rewardedId,
                        onRewardEarned = { onRewardEarned?.invoke() },
                        sessionId = sessionId
                    )
                }
    ...
    suspend fun showNativeAdInContainer(
        ...
        // 记录广告位置，生成 session_id
        val sessionId = AdPositionTracker.trackNativeAdPosition(position)
        ...
        val winner = NativeBiddingManager.bidding(context, position = position)
        ...
        return when (winner) {
            ...
            BiddingWinner.GAM -> {
                AdLogger.d("使用 GAM 展示原生广告")
                val style = if (styleType == NativeAdStyleType.STANDARD) BillConfig.gam.nativeStyleStandard else BillConfig.gam.nativeStyleLarge
                AdEventReporter.withLoadPosition(position) {
                    GamNativeAdController.getInstance().showAdInContainer(
                        context,
                        container,
                        style,
                        BillConfig.gam.nativeId,
                        sessionId = sessionId
                    )
                }
            }
    ...
    suspend fun showFullScreenNativeAdInContainer(
        ...
        // 记录广告位置，生成 session_id
        val sessionId = AdPositionTracker.trackFullNativeAdPosition(position)

        // 检查是否可以展示（全局开关 + 总控限制）
        val checkResult = checkShowEligibility(AdType.FULL_SCREEN_NATIVE, "全屏原生广告", sessionId)
        if (checkResult != null) return checkResult

        val winner = FullScreenNativeBiddingManager.bidding(activity, position = position)
            ...
            coroutineScope {
                ...
                val fullNativeDeferred = async(start = CoroutineStart.UNDISPATCHED) {
                    when (winner) {
                        ...
                        BiddingWinner.GAM -> {
                            AdLogger.d("使用 GAM 展示全屏原生广告")
                            AdEventReporter.withLoadPosition(position) {
                                GamFullScreenNativeAdActivity.start(
                                    activity = activity,
                                    sessionId = sessionId,
                                    onAdDisplayedCallback = onFullNativeDisplayed
                                )
    ...
    suspend fun showBannerAd(
        ...
        // 记录广告位置，生成 session_id
        val sessionId = AdPositionTracker.trackBannerAdPosition(position)
        ...
        val winner = BannerBiddingManager.bidding(activity, position = position)
        ...
        return when (winner) {
            ...
            BiddingWinner.GAM -> {
                AdLogger.d("使用 GAM 展示Banner广告")
                AdEventReporter.withLoadPosition(position) {
                    GamBannerAdController.getInstance().showAd(
                        activity,
                        container,
                        BillConfig.gam.bannerId,
                        sessionId = sessionId
                    )
                }
            }
    ...
    fun isAnyInterstitialOrFullScreenNativeShowing(): Boolean {
        return AdmobInterstitialAdController.getInstance().isAdShowing() ||
            ...
            GamInterstitialAdController.getInstance().isAdShowing() ||
            GamFullScreenNativeAdController.getInstance().isAdShowing() ||
    ...
    fun hasAnyFullScreenNativeCached(): Boolean {
        return when (AdSourceController.getCurrentSource()) {
            ...
            AdSourceController.AdSource.GAM -> GamFullScreenNativeAdController.getInstance().hasCachedAd()
            ...
            AdSourceController.AdSource.BIDDING -> AdmobFullScreenNativeAdController.getInstance().hasCachedAd() ||
                GamFullScreenNativeAdController.getInstance().hasCachedAd() ||
    ...
    private fun hasAnyAppOpenCached(): Boolean {
        ...
        val gamEnabled = BiddingPlatformController.isGamEnabled(AdType.APP_OPEN) && BiddingExclusionController.canPlatformBidSilent(AdType.APP_OPEN, AdPlatform.GAM)
        ...
        if (gamEnabled && GamAppOpenAdController.getInstance().getCachedAdPeek(BillConfig.gam.splashId) != null) return true
    ...
    private fun hasAnyRewardedCached(): Boolean {
        ...
        val gamEnabled = BiddingPlatformController.isGamEnabled(AdType.REWARDED) && BiddingExclusionController.canPlatformBidSilent(AdType.REWARDED, AdPlatform.GAM)
        ...
        if (gamEnabled && GamRewardedAdController.getInstance().peekCachedAd(BillConfig.gam.rewardedId) != null) return true
    ...
    private fun hasAnyInterstitialCached(): Boolean {
        ...
        val gamEnabled = BiddingPlatformController.isGamEnabled(AdType.INTERSTITIAL) && BiddingExclusionController.canPlatformBidSilent(AdType.INTERSTITIAL, AdPlatform.GAM)
        ...
        if (gamEnabled && GamInterstitialAdController.getInstance().getCachedAdPeek(BillConfig.gam.interstitialId) != null) return true
    ...
    private fun buildNoPlatformAvailableReason(adType: AdType): String {
        val platformDetails = AdPlatform.entries.joinToString(";") { platform ->
            ...
            if (missingFields.isNotEmpty()) {
                return@joinToString "${platform.key}:missing_runtime_config(${missingFields.joinToString(",")})"
            }
    ...
    private fun isPlatformInitialized(platform: AdPlatform): Boolean {
        return when (platform) {
            ...
            AdPlatform.GAM -> GamManager.isInitialized()
