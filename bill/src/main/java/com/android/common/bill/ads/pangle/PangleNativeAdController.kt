...
// 注册 Activity 销毁时的清理回调
                        ...
                        interactionListener = object : PAGNativeAdInteractionCallback() {
                            ...
                            override fun onAdShowed() {
                                ...
                                if (!isCacheFull(finalAdUnitId)) {
                                    val reloadSessionId = showSessionId
                            ...
                            override fun onAdClicked() {
                                ...
                                AdEventReporter.reportClick(AdType.NATIVE, AdPlatform.PANGLE, finalAdUnitId, currentAdUniqueId, totalClickCount, currentAdSource ?: AdPlatform.PANGLE.key, nativeAd.pagRevenueInfo?.showEcpm?.revenue?.toDoubleOrNull() ?: 0.0, currentCurrency ?: "USD", sessionId = showSessionId)
                            }

                            override fun onAdDismissed() {
                                AdLogger.d("Pangle原生广告关闭")
                                totalCloseCount++
                                AdEventReporter.reportClose(AdType.NATIVE, AdPlatform.PANGLE, finalAdUnitId, totalCloseCount, currentAdSource ?: AdPlatform.PANGLE.key, nativeAd.pagRevenueInfo?.showEcpm?.revenue?.toDoubleOrNull() ?: 0.0, currentCurrency ?: "USD", sessionId = showSessionId)
                            }
    ...
    fun getCachedAdPeek(adUnitId: String? = null): PAGNativeAd? {
        return getCurrentAd(adUnitId)
    }
